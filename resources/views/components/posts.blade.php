<li class="flex items-center justify-between border-gray-200">
    ...
    <div class="md:flex items-center text-xs md:space-x-2">
        <span class="mt-1">Atsakė:</span>
        <img class="w-6 h-6 mt-1 ml-2 md:ml-0 rounded-full" src="{{$post->user->avatar}}" alt="{{ $post->user->username }}">
        <a class="mt-1 font-semibold" href="/profile/{{$post->user->slug}}" title="Narys: {{$post->user->username}}">{{ $post->user->username }}</a>
    </div>
</li>
